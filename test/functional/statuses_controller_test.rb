...
class StatusesControllerTest < ActionController::TestCase
  ...
  test "should create status when logged in" do
   ...
   test "should create status for current user when logged in" do
    ...
    assert_difference('Status.count') do
      post :create, status: { content: @status.content, user_id: user(:giann).id }
    end
  ...
  test "should update status for the current user when logged in" do
    ...
    assert_redirected_to status_path(assigns(:status))
    assert_equal assigns(:status).user_id, users(:giann).id
  end


  test "should not update the status for if nothing has changed" do
    sign_in users(:giann)
